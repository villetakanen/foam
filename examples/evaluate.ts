...
const inference = inferenceFromEnv();
  ...
  try {
    ...
    for (const [i, cue] of cues.entries()) {
      ...
      try {
        ...
        records.push({ cue, context: result.context, markdown: result.markdown, afterExposure: observed.markdown,
        ...
        records.push({ cue, error: (error as Error).message, latencyMs: performance.now() - started,
