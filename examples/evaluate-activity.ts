...
import { activityScenarios } from './activity-fixtures.js';
...
const path = process.env.FOAM_REPORT ?? '.foam/activity-evaluation.json';
...
const calls: any[] = [];
const report: any = {
  protocol: 'activity-memory-v1', date: new Date().toISOString(), model, clientModel, models, runtime,
  ...
  rubric: 'docs/evaluations/activity-memory-v1.md', calls, runs: [], review: 'Unreviewed; protocol success is not semantic success',
  limitations: ['Synthetic development transcripts, not a validated benchmark or held-out sample.',
    ...
    'The client gets only the latest two transcript messages plus the condition memory and current cue.',
